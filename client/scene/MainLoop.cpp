...
void MainLoop::Draw()
{
    ...
    if (snapshot_) {
        ...
        info_json += "}";
        _stprintf(tmp_str, _T("{\"private\":[%d,%d],\"body\":\"Screenshot saved: %03d.png\"}"), id, id, snapshot_number_);
    ...
    if (const auto& channel = command_manager_->current_channel()) {
        BOOST_FOREACH(const auto& warp_point, channel->warp_points) {
            ...
            if (world_manager_->stage()->IsVisiblePoint(point)) {
                ...
                auto text = unicode::ToTString(warp_point.name);
                if (distance < 50) {
                    text += _T("\nPress M to warp!");
    ...
    if (command_manager_->status() == CommandManager::STATUS_ERROR){
        ...
        auto str = unicode::ToTString(_T("Disconnected from the server."));
